...
public class SplashActivity extends AppCompatActivity implements IDownloaderClient {
  ...
  boolean expansionFilesDelivered() {
    Log.i(TAG, "Seeing if expansion files are delivered.");
    for (XAPKFile xf : xAPKS) {
      String fileName = Helpers.getExpansionAPKFileName(this, xf.mIsMain, xf.mFileVersion);
      Log.i(TAG, "Filename: " + fileName);
      Log.i(TAG, "Path: " + Helpers.getSaveFilePath(this));
      if (!Helpers.doesFileExist(this, fileName, xf.mFileSize, false)) {
        Log.i(TAG, "oh no.. returning false sadfaceness");
        return false;
      }
    }
    return true;
  ...
  void validateXAPKZipFiles() {
    AsyncTask<Object, DownloadProgressInfo, Boolean> validationTask =
      ...
      protected Boolean doInBackground(Object... params) {
        for (XAPKFile xf : xAPKS) {
          String fileName =
              Helpers.getExpansionAPKFileName(SplashActivity.this, xf.mIsMain, xf.mFileVersion);
                    ...
                    if (timePassed > 0) {
                      ...
                      if (0 != averageVerifySpeed) {
                        averageVerifySpeed = SMOOTHING_FACTOR * currentSpeedSample
                            + (1.f - SMOOTHING_FACTOR) * averageVerifySpeed;
                      } else {
